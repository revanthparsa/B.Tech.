...
import csv
import math
with open("copper_top_square.gbr","r") as input:
    with open("square1.txt","w") as output: 
        for line in input:
            for p in line:
                if p =='X':
                    output.write(line)
                    ...
                    break            
for i, line in enumerate(fileinput.input('square1.txt', inplace=1)):
...
with open('square.txt','w') as out:
    for s in range(0,len(xco)):
        xco[s]=float(xco[s])/10000.0
        yco[s]=float(yco[s])/10000.0
